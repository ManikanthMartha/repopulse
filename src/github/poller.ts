...
import { getUserRepos, getRepoState, updateRepoState, getSubscriptionByRepo, upsertRepository, getUserIdByTelegramId } from '../db/queries';
...
async function fetchIssues(repoFullName: string, telegramId: number, since?: string): Promise<any[]> {
...
async function fetchPRs(repoFullName: string, telegramId: number, since?: string): Promise<any[]> {
  ...
  for (const user of users) {
    ...
    for (const repoFullName of repoNames) {
      try {
        ...
        log(`[Poller] User ${userId}, Repo ${repoFullName} (ID: ${repoId}) lastIssueCheck: ${lastIssueCheck}, lastPRCheck: ${lastPRCheck}`);

        // Fetch new/updated issues using user's token
        const issues = await fetchIssues(repoFullName, telegramId, lastIssueCheck);
        ...
        log(`[Poller] Updated user_repo_state for user ${userId}, repo ${repoFullName}`);
      } catch (err) {
        logError(`[Poller] Error processing repo ${repoFullName} for user ${telegramId}:`, err);
      }
    }
  }

...
export async function saveSubscription(
  ...
  `;
  const result = await query<SubscriptionRecord>(sql, [
    userId,
    repoId,
...
export async function getRepoState(
  ...
  return {
    user_id: row.user_id,
    repo_id: row.repo_id,
    last_issue_check: row.last_issue_check
      ? new Date(row.last_issue_check)
      : null,
...
export async function updateRepoState(
  ...
  await query(sql, [
    userId,
    repoId,
    lastIssueCheck.toISOString(),
    lastPRCheck.toISOString(),
  ]);
}

export async function setInitialRepoState(
  userId: number,
  repoId: number,
  lastIssueCheck: Date,
  lastPRCheck: Date,
...
export async function getRepoIdByFullName(
  fullName: string,
): Promise<number | null> {

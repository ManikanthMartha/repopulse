...
export const connectCommand = async (
  ...
  githubAuthUrl.searchParams.set(
    "redirect_uri",
    config.GITHUB_OAUTH_CALLBACK_URL,
  );
  ...
  return bot.sendMessage(
    ...
    `🔐 *Connect your GitHub Account*\n\n` +
      `*Free Tier (No Connection):*\n` +
      `• Track up to 5 repositories\n` +
      `• Uses shared API quota\n\n` +
      `*Connected (Unlimited):*\n` +
...
export const disconnectCommand = async (
  ...
  return bot.sendMessage(
    ...
    `⚠️ *Disconnect GitHub?*\n\n` +
      ...
      `• Limit you to 5 repos (you may need to unsubscribe from some)\n` +
  ...
  if (user.is_connected) {
    return bot.sendMessage(
      ...
      `📊 *Your RepoPulse Status*\n\n` +
        ...
        `📁 Repos tracked: ${subCount} (Unlimited)\n` +
        `✨ No tracking limits!\n\n` +
        `Commands:\n` +
